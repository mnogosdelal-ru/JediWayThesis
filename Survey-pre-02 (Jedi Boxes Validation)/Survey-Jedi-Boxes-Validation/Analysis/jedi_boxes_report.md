# Отчёт анализа данных опроса "Три коробочки"

**Дата генерации:** 2026-04-06 13:29
**Всего респондентов:** 370
**Завершили опрос:** 231

---

## Описание исследования

Методика "Три коробочки" — ипсативный инструмент распределения ограниченного 
ресурса (6 единиц энергии) между тремя функциональными категориями:

- 🔴 **Срочное (Реактивность)** — задачи, требующие немедленного решения
- 🟢 **Целевое (Проактивность)** — задачи, приближающие к долгосрочным целям
- ⚪ **Операционное (Поддержание)** — рутинные задачи и восстановление

Цель исследования — валидация инструмента через анализ связи с продуктивностью,
выгоранием и удовлетворённостью жизнью.

---


## Описательная статистика

**Демография завершённых анкет:**

- Возраст: M = 37.1, SD = 8.2, диапазон = 19-61, n = 229

- Пол: {'male': 167, 'female': 62}

- Должности: {'Старший специалист': 57, 'Специалист': 43, 'Менеджер среднего звена': 43, 'Тимлид': 25, 'Высший менеджмент': 17, 'Фрилансер': 15, 'Владелец бизнеса': 15, 'Другое': 5, 'Безработный': 3, 'Школьник / Студент': 3, 'Младший специалист': 3}


**Распределение кубиков по зонам:**

- Срочное ●: M = 1.77, SD = 1.00, диапазон = 0-4

- Целевое ●: M = 1.90, SD = 1.04, диапазон = 0-4

- Операционное ●: M = 2.30, SD = 0.93, диапазон = 0-6


**Распределение уровней профиля:**

| Уровень | Профиль | n | % |
| --- | --- | --- | --- |
| 1 | Хаос (все зоны равны) | 20 | 8.7% |
| 2 | Выживание (К>З>С) | 21 | 9.1% |
| 3 | Кризис (К>С>З или С>К>З) | 71 | 30.7% |
| 4 | Не сдаёмся (З>К>С) | 20 | 8.7% |
| 5 | Рост (З>С>К) | 48 | 20.8% |
| 6 | Дзен (С>З>К) | 51 | 22.1% |


**Шкалы валидации:**

- Прокрастинация: M = 25.7, SD = 7.2, диапазон = 0-40

- SWLS (удовлетворённость): M = 22.2, SD = 6.6, диапазон = 0-34

- MBI (выгорание): M = 23.7, SD = 8.9, диапазон = 6-47


## Визуализации распределений

**Контекстуальные показатели:**

![Баланс работа/личное](figures/work_life_balance.png)

![Типичность недели](figures/week_typicality.png)

![Энергетический дефицит](figures/energy_deficit.png)

![Распределение кубиков](figures/cubes_distribution.png)

![Уровни профиля](figures/profile_levels.png)

![Распределение по полу](figures/gender_distribution.png)

![Распределение по возрасту](figures/age_distribution.png)

![Корреляционная матрица](figures/correlation_matrix.png)

*Значимость: *** p<0.001, ** p<0.01, * p<0.05, n.s. - не значимо | eps=1*

![Шкалы](figures/scale_distributions.png)


## Проверка гипотез исследования


### H0: Различия между уровнями профиля

Различия между уровнями 1-6 по MBI, прокрастинации и SWLS


**Прокрастинация:**

**Односторонний ANOVA**:

- Статистика: 5.222

- p-value: 0.0001

- Размер эффекта: 0.104

**Вывод**: Результат значим (p < 0.05). Подтверждается

*Post-hoc сравнения доступны в детальном анализе*

| Уровень | M | SD | n |
| --- | --- | --- | --- |
| 1 | 23.6 | 9.1 | 20 |
| 2 | 24.6 | 7.4 | 21 |
| 3 | 28.8 | 6.2 | 71 |
| 4 | 26.2 | 7.3 | 20 |
| 5 | 22.6 | 6.1 | 48 |
| 6 | 25.3 | 7.2 | 51 |


**SWLS:**

**Односторонний ANOVA**:

- Статистика: 1.068

- p-value: 0.3788

- Размер эффекта: 0.023

**Вывод**: Результат не значим (p > 0.05). Не подтверждается

| Уровень | M | SD | n |
| --- | --- | --- | --- |
| 1 | 21.4 | 9.5 | 20 |
| 2 | 22.3 | 4.9 | 21 |
| 3 | 21.1 | 6.3 | 71 |
| 4 | 24.4 | 5.6 | 20 |
| 5 | 23.0 | 6.5 | 48 |
| 6 | 22.4 | 6.7 | 51 |


**MBI (выгорание):**

**Односторонний ANOVA**:

- Статистика: 6.105

- p-value: 0.0000

- Размер эффекта: 0.119

**Вывод**: Результат значим (p < 0.05). Подтверждается

*Post-hoc сравнения доступны в детальном анализе*

| Уровень | M | SD | n |
| --- | --- | --- | --- |
| 1 | 21.6 | 10.1 | 20 |
| 2 | 24.7 | 6.2 | 21 |
| 3 | 27.8 | 8.0 | 71 |
| 4 | 21.6 | 6.9 | 20 |
| 5 | 19.8 | 8.5 | 48 |
| 6 | 22.9 | 9.4 | 51 |


### H1: Корреляция 🔴 с валидационными шкалами

🔴 положительно коррелирует с прокрастинацией и MBI

**🔴 (0-4) vs Прокрастинация (0-40):**

Коэффициент корреляции r = 0.230, p = 0.0004

Диапазон значений: 🔴 [0; 4], Прокрастинация [0; 40]

**Вывод**: Подтверждается (положительная связь)

**🔴 (0-4) vs MBI (выгорание) (6-47):**

Коэффициент корреляции r = 0.233, p = 0.0003

Диапазон значений: 🔴 [0; 4], MBI (выгорание) [6; 47]

**Вывод**: Подтверждается (положительная связь)


**🔴 (0-4) vs SWLS (удовлетворённость) (0-34) (дополнительно):**

Коэффициент корреляции r = -0.047, p = 0.4746

Диапазон значений: 🔴 [0; 4], SWLS (удовлетворённость) [0; 34]

**Вывод**: Связь не обнаружена


### H2: Корреляция 🟢 с валидационными шкалами

🟢 положительно коррелирует с SWLS и отрицательно с MBI

**🟢 (0-4) vs SWLS (0-34):**

Коэффициент корреляции r = 0.210, p = 0.0013

Диапазон значений: 🟢 [0; 4], SWLS [0; 34]

**Вывод**: Подтверждается (положительная связь)


**🟢 (0-4) vs MBI (6-47):**

Коэффициент корреляции r = -0.377, p = 0.0000

Диапазон значений: 🟢 [0; 4], MBI [6; 47]

**Вывод**: Подтверждается (отрицательная связь)


**🟢 (0-4) vs Прокрастинация (0-40) (дополнительно):**

Коэффициент корреляции r = -0.285, p = 0.0000

Диапазон значений: 🟢 [0; 4], Прокрастинация [0; 40]

**Вывод**: Обнаружена отрицательная связь


### H3: Низкое 🔴 и выгорание

Низкое 🔴 связано с более низким выгоранием

- Низкое 🔴 (<2): n = 108, M = 21.5, SD = 8.8

- Высокое 🔴 (≥2): n = 123, M = 25.6, SD = 8.5

**U-критерий Манна-Уитни**:

- Статистика: 4662.000

- p-value: 0.0001

**Вывод**: Результат значим (p < 0.05). Подтверждается


### H4: Доминирование 🟢 и удовлетворённость

Профили с доминированием 🟢 имеют высокие баллы SWLS

- Уровни 5-6 (доминирование 🟢): n = 99, M = 22.7, SD = 6.6

- Другие уровни: n = 132, M = 21.8, SD = 6.6

**U-критерий Манна-Уитни**:

- Статистика: 7080.000

- p-value: 0.1386

**Вывод**: Результат не значим (p > 0.05). Не подтверждается


### H5: Модерация записями

Модерация записями: восстановление по записям vs памяти

- По записям (4-5): n = 25

- По памяти (1-2): n = 190

| Зона | По записям | По памяти | Разница | p-value |
| --- | --- | --- | --- | --- |
| Целевое ● | 2.36 | 1.82 | +0.54 | p=0.019* |
| Срочное ● | 1.40 | 1.86 | -0.46 | p=0.023* |
| Операционное ● | 2.24 | 2.32 | -0.08 | p=0.661 |


Прокрастинация: по записям = 22.8, по памяти = 26.5 (p=0.017*)

MBI: по записям = 21.3, по памяти = 24.2 (p=0.098)


*Значимость: * p<0.05*


### H6: Модерация 🔴

Модерация красным: связь ⚪ с прокрастинацией усиливается при высоком 🔴

- Низкое 🔴 (0-1): корреляция ⚪-прокрастинация r = 0.195, p = 0.0429

- Высокое 🔴 (3+): корреляция ⚪-прокрастинация r = 0.347, p = 0.0070

**Вывод**: Подтверждается (связь сильнее при высоком 🔴)


### H7: Криволинейная связь 🟢 и MBI

Криволинейная связь 🟢 и выгорания (U-образная)

- Линейная модель: r = -0.367, p = 0.0000

![Криволинейная связь](figures/green_burnout_curvilinear.png)

*Требуется большая выборка для надёжной проверки U-образной зависимости*


### H8: Модерация балансом работа/личное

Баланс работа/личное как модератор связи 🔴 и выгорания

- Больше работы (work_life < 0): корреляция 🔴-MBI r = 0.085, p = 0.4925

- Больше личного (work_life > 0): корреляция 🔴-MBI r = 0.245, p = 0.0061

**Вывод**: Не подтверждается


### H9: Дефицит как медиатор

Энергетический дефицит как медиатор

- Прямая связь 🔴 → MBI: r = 0.233, p = 0.0003

- Связь 🔴 → дефицит: r = 0.272, p = 0.0000

- Связь дефицит → MBI: r = 0.362, p = 0.0000

*Полный медиационный анализ требует большей выборки*


### H10: Гендерные различия

Гендерные различия в распределении зон

- Мужчины: n = 167

- Женщины: n = 62

| Зона | Мужчины (M) | Женщины (M) | Разница | p-value |
| --- | --- | --- | --- | --- |
| Срочное ● | 1.78 | 1.74 | +0.04 | p=0.602 |
| Целевое ● | 1.91 | 1.87 | +0.04 | p=0.959 |
| Операционное ● | 2.27 | 2.39 | -0.12 | p=0.406 |

**Вывод**: Не подтверждается — нет значимых гендерных различий


### H11: Возрастной тренд

Возрастной тренд: с возрастом доля 🔴 снижается

- Срочное ●: r = nan, p = nan  — с возрастом растёт

- Целевое ●: r = nan, p = nan  — с возрастом растёт

- Операционное ●: r = nan, p = nan  — с возрастом растёт


### H12: Профиль Дзен и дефицит

Профиль "Дзен" связан с наименьшим энергетическим дефицитом

- Уровень 6 (Дзен): n = 51, M = 2.49

- Другие уровни: n = 180, M = 3.53

**U-критерий Манна-Уитни**:

- Статистика: 3606.000

- p-value: 0.0070

**Вывод**: Результат значим (p < 0.05). Подтверждается


### H7a: Линейная регрессия - предсказание по кубикам

Линейная регрессия: MBI, Прокрастинация и SWLS предсказываются числом красных и зеленых кубиков

**MBI (выгорание):**

- Уравнение: MBI (выгорание) = 29.68 + -0.03×Срочное + -3.13×Целевое

- R² = 0.135 (13.5% дисперсии объясняется моделью)

- Коэффициенты:

  - Intercept: 29.68 (p=0.000)

  - Срочное (🔴): -0.03 (p=0.969)

  - Целевое (🟢): -3.13 (p=0.000)

- Предсказательная способность модели: умеренная (R²=0.135)

![MBI (выгорание)](figures/regression_mbi_total.png)

**Прокрастинация:**

- Уравнение: Прокрастинация = 26.75 + 0.80×Срочное + -1.31×Целевое

- R² = 0.072 (7.2% дисперсии объясняется моделью)

- Коэффициенты:

  - Intercept: 26.75 (p=0.000)

  - Срочное (🔴): 0.80 (p=0.153)

  - Целевое (🟢): -1.31 (p=0.015)

- Предсказательная способность модели: слабая (R²=0.072)

![Прокрастинация](figures/regression_proc_total.png)

**SWLS (удовлетворённость):**

- Уравнение: SWLS (удовлетворённость) = 16.99 + 0.98×Срочное + 1.83×Целевое

- R² = 0.057 (5.7% дисперсии объясняется моделью)

- Коэффициенты:

  - Intercept: 16.99 (p=0.000)

  - Срочное (🔴): 0.98 (p=0.057)

  - Целевое (🟢): 1.83 (p=0.000)

- Предсказательная способность модели: слабая (R²=0.057)

![SWLS (удовлетворённость)](figures/regression_swls_total.png)


### H7b: Предсказание MBI и SWLS только по целевым кубикам (🟢)


**H7b: MBI и SWLS предсказываются только числом зелёных (целевых) кубиков.**

Целевые кубики отражают проактивное распределение энергии — направление ресурсов
на задачи, приближающие к долгосрочным целям. Гипотеза: именно проактивность
является основным предиктором благополучия и выгорания.



**MBI (выгорание):**

- Уравнение: MBI (выгорание) = 29.60 + -3.12 × 🟢

- R² = 0.135 (13.5% дисперсии объясняется)

- Коэффициент 🟢: -3.12 (p=0.0000)

- Предсказательная способность: умеренная

- Интерпретация: чем больше целевых кубиков, тем ниже MBI (выгорание)


**SWLS (удовлетворённость):**

- Уравнение: SWLS (удовлетворённость) = 19.76 + 1.29 × 🟢

- R² = 0.042 (4.2% дисперсии объясняется)

- Коэффициент 🟢: 1.29 (p=0.0018)

- Предсказательная способность: слабая

- Интерпретация: чем больше целевых кубиков, тем выше SWLS (удовлетворённость)


**Сводная таблица регрессий (только 🟢):**

| Показатель | Intercept | β (🟢) | R² | p (β) | Значимость β |
| --- | --- | --- | --- | --- | --- |
| MBI (выгорание) | 29.60 | -3.117 | 0.135 | 0.0000 | *** |
| SWLS (удовлетворённость) | 19.76 | 1.292 | 0.042 | 0.0018 | ** |


![H7b: Регрессии только по 🟢](figures/h7b_proactive_only.png)


**Выводы по H7b:**

- MBI (выгорание): R² = 0.135, β = -3.12, p = 0.0000 (значим)

- SWLS (удовлетворённость): R² = 0.042, β = 1.29, p = 0.0018 (значим)


### H7c: Предсказание шкал по всем трём типам кубиков (🔴, 🟢, ⚪)


**H7c: MBI, Прокрастинация и SWLS предсказываются числом всех трёх типов кубиков.**

В отличие от H7a (только 🔴 и 🟢), здесь включены все три зоны, включая операционное (⚪).
Это позволяет оценить независимый вклад каждой зоны.



**MBI (выгорание):**

- Уравнение: MBI (выгорание) = 31.00 + -0.24×🔴 + -3.34×🟢 + -0.24×⚪

- R² = 0.135 (13.5% дисперсии объясняется)

- Коэффициенты:

  - 🔴: β = -0.238, p = 0.8692 n.s.

  - 🟢: β = -3.343, p = 0.0206 *

  - ⚪: β = -0.237, p = 0.8688 n.s.

- Предсказательная способность: умеренная


**Прокрастинация:**

- Уравнение: Прокрастинация = 0.00 + 5.07×🔴 + 2.99×🟢 + 4.79×⚪

- R² = 0.136 (13.6% дисперсии объясняется)

- Коэффициенты:

  - 🔴: β = 5.073, p = 0.0000 ***

  - 🟢: β = 2.986, p = 0.0110 *

  - ⚪: β = 4.790, p = 0.0001 ***

- Предсказательная способность: умеренная


**SWLS (удовлетворённость):**

- Уравнение: SWLS (удовлетворённость) = -0.00 + 3.70×🔴 + 4.55×🟢 + 3.04×⚪

- R² = 0.088 (8.8% дисперсии объясняется)

- Коэффициенты:

  - 🔴: β = 3.699, p = 0.0009 ***

  - 🟢: β = 4.553, p = 0.0000 ***

  - ⚪: β = 3.042, p = 0.0060 **

- Предсказательная способность: слабая


**Сводная таблица множественной регрессии (🔴, 🟢, ⚪):**

| Показатель | Intercept | β (🔴) | β (🟢) | β (⚪) | R² | p (🔴) | p (🟢) | p (⚪) |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| MBI (выгорание) | 31.00 | -0.238 | -3.343 | -0.237 | 0.135 | 0.8692 n.s. | 0.0206 * | 0.8688 n.s. |
| Прокрастинация | 0.00 | 5.073 | 2.986 | 4.790 | 0.136 | 0.0000 *** | 0.0110 * | 0.0001 *** |
| SWLS (удовлетворённость) | -0.00 | 3.699 | 4.553 | 3.042 | 0.088 | 0.0009 *** | 0.0000 *** | 0.0060 ** |


**Сравнение H7a (🔴+🟢) и H7c (🔴+🟢+⚪):**

*Показывает, добавляет ли операционная зона объяснительную силу.*

| Показатель | R² (H7a: 🔴+🟢) | R² (H7c: 🔴+🟢+⚪) | ΔR² |
| --- | --- | --- | --- |
| MBI (выгорание) | 0.135 | 0.135 | +0.000 |
| Прокрастинация | 0.072 | 0.136 | +0.064 |
| SWLS (удовлетворённость) | 0.057 | 0.088 | +0.031 |


![H7c: Сравнение R²](figures/h7c_comparison.png)


**Выводы по H7c:**

- Добавление операционной зоны (⚪) в модель меняет объяснительную силу

- MBI (выгорание): ΔR² = +0.000 при добавлении ⚪

- Прокрастинация: ΔR² = +0.064 при добавлении ⚪

- SWLS (удовлетворённость): ΔR² = +0.031 при добавлении ⚪


### H7d: Предсказание по всем трём кубикам на типовой неделе


**H7d: H7c на подвыборке респондентов с типовой неделей (representative ∈ [-1, 1]).**

Проверяем, меняется ли предсказательная сила модели при фильтрации респондентов,
у которых неделя была близка к обычной. Это снижает шум от нетипичных обстоятельств.


**Полная выборка:** 231 респондентов

**Типовая неделя:** 154 респондентов (66.7%)


**MBI (выгорание) (типовая неделя):**

- Уравнение: MBI (выгорание) = 31.00 + -0.23×🔴 + -3.49×🟢 + -0.48×⚪

- R² = 0.159 (15.9% дисперсии объясняется)

- Коэффициенты:

  - 🔴: β = -0.227, p = 0.8689 n.s.

  - 🟢: β = -3.487, p = 0.0117 *

  - ⚪: β = -0.477, p = 0.7278 n.s.

- Предсказательная способность: умеренная


**Прокрастинация (типовая неделя):**

- Уравнение: Прокрастинация = 0.00 + 5.22×🔴 + 2.73×🟢 + 4.87×⚪

- R² = 0.195 (19.5% дисперсии объясняется)

- Коэффициенты:

  - 🔴: β = 5.224, p = 0.0000 ***

  - 🟢: β = 2.731, p = 0.0200 *

  - ⚪: β = 4.869, p = 0.0000 ***

- Предсказательная способность: умеренная


**SWLS (удовлетворённость) (типовая неделя):**

- Уравнение: SWLS (удовлетворённость) = -0.00 + 3.70×🔴 + 4.41×🟢 + 3.21×⚪

- R² = 0.091 (9.1% дисперсии объясняется)

- Коэффициенты:

  - 🔴: β = 3.700, p = 0.0019 **

  - 🟢: β = 4.414, p = 0.0002 ***

  - ⚪: β = 3.208, p = 0.0066 **

- Предсказательная способность: слабая


**Сводная таблица множественной регрессии (типовая неделя):**

| Показатель | Intercept | β (🔴) | β (🟢) | β (⚪) | R² | p (🔴) | p (🟢) | p (⚪) |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| MBI (выгорание) | 31.00 | -0.227 | -3.487 | -0.477 | 0.159 | 0.8689 n.s. | 0.0117 * | 0.7278 n.s. |
| Прокрастинация | 0.00 | 5.224 | 2.731 | 4.869 | 0.195 | 0.0000 *** | 0.0200 * | 0.0000 *** |
| SWLS (удовлетворённость) | -0.00 | 3.700 | 4.414 | 3.208 | 0.091 | 0.0019 ** | 0.0002 *** | 0.0066 ** |


**Сравнение H7c (полная) и H7d (типовая неделя):**

| Показатель | R² (полная) | R² (типовая) | ΔR² | β🔴 полная | β🔴 типовая | β🟢 полная | β🟢 типовая | β⚪ полная | β⚪ типовая |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| MBI (выгорание) | 0.135 | 0.159 | +0.024 | -0.238 | -0.227 | -3.343 | -3.487 | -0.237 | -0.477 |
| Прокрастинация | 0.136 | 0.195 | +0.059 | 5.073 | 5.224 | 2.986 | 2.731 | 4.790 | 4.869 |
| SWLS (удовлетворённость) | 0.088 | 0.091 | +0.003 | 3.699 | 3.700 | 4.553 | 4.414 | 3.042 | 3.208 |


![H7d: Сравнение](figures/h7d_comparison.png)


**Выводы по H7d:**

- При фильтрации по типовой неделе:

- MBI (выгорание): R² изменяется с 0.135 на 0.159 (Δ = +0.024)

- Прокрастинация: R² изменяется с 0.136 на 0.195 (Δ = +0.059)

- SWLS (удовлетворённость): R² изменяется с 0.088 на 0.091 (Δ = +0.003)


### H10a: Владельцы бизнеса и высшее руководство

Владельцы бизнеса и высшее руководство отличаются по распределению кубиков от остальных

- Владельцы бизнеса и высшее руководство: n = 32

- Остальные: n = 199

| Зона | Руководство (M) | Остальные (M) | Разница | p-value |
| --- | --- | --- | --- | --- |
| Срочное ● | 1.53 | 1.81 | -0.28 | p=0.161 |
| Целевое ● | 2.09 | 1.87 | +0.22 | p=0.249 |
| Операционное ● | 2.38 | 2.29 | +0.09 | p=0.301 |


**Валидационные шкалы:**

- Прокрастинация: руководство = 23.4, остальные = 26.1 (p=0.074)

- SWLS: руководство = 24.2, остальные = 21.9 (p=0.076)

- MBI: руководство = 21.0, остальные = 24.1 (p=0.096)


**Вывод**: Не подтверждается — нет значимых различий между группами


## Анализ индекса ProductivityIndex (log₂(Целевое/Срочное))


**Описание индекса:**

ProductivityIndex — это логарифм по основанию 2 отношения количества кубиков в зоне "Целевое" к количеству
кубиков в зоне "Срочное" с добавлением небольшого смещения (eps) для избежания
деления на ноль и логарифмирования нуля.

Формула: ProductivityIndex = log₂((Целевое + eps) / (Срочное + eps))

Интерпретация:
- ProductivityIndex > 0: преобладание проактивности (целевые задачи доминируют)
- ProductivityIndex ≈ 0: баланс между срочными и целевыми задачами
- ProductivityIndex < 0: преобладание реактивности (срочные задачи доминируют)

Преимущество логарифмической шкалы по основанию 2:
- Симметрия: отношения 2:1 и 1:2 дают равные по модулю, но противоположные по знаку значения (+1 и -1)
- Интерпретируемость: значение +1 означает в 2 раза больше целевых, -1 — в 2 раза больше срочных
- При 7 кубиках и eps=1 диапазон составляет [-3, 3]

Высокий ProductivityIndex указывает на более продуктивный профиль распределения энергии.

**Параметр смещения:** eps = 1 (диапазон при 7 кубиках: [-3, 3])


**Описательная статистика ProductivityIndex:**

- M = 0.06, SD = 0.98

- Медиана = 0.00

- Диапазон = -2.32 – 2.32


**Распределение по категориям:**

- Преобладание проактивности (PI > 0.585): 58 (25.1%)

- Баланс (-0.585 ≤ PI ≤ 0.585): 124 (53.7%)

- Преобладание реактивности (PI < -0.585): 49 (21.2%)


**Корреляции ProductivityIndex с валидационными шкалами:**

- Прокрастинация: r = -0.287, p = 0.0000 * (слабая отрицательная)

- SWLS: r = 0.153, p = 0.0199 * (слабая положительная)

- MBI: r = -0.348, p = 0.0000 * (умеренная отрицательная)


![Распределение ProductivityIndex](figures/productivity_index_distribution.png)


**Линейная регрессия с ProductivityIndex:**

*Модель: Зависимая переменная = β₀ + β₁ × ProductivityIndex*


**MBI (выгорание):**

- Уравнение: MBI (выгорание) = 23.86 + -3.00 × ProductivityIndex

- R² = 0.111 (11.1% дисперсии объясняется)

- Коэффициент ProductivityIndex: -3.00 (p=0.000)

- Предсказательная способность: умеренная (R²=0.111)

- Интерпретация: чем выше ProductivityIndex (больше проактивности), тем ниже MBI (выгорание)


**Прокрастинация:**

- Уравнение: Прокрастинация = 25.81 + -2.04 × ProductivityIndex

- R² = 0.077 (7.7% дисперсии объясняется)

- Коэффициент ProductivityIndex: -2.04 (p=0.000)

- Предсказательная способность: слабая (R²=0.077)

- Интерпретация: чем выше ProductivityIndex (больше проактивности), тем ниже Прокрастинация


**SWLS (удовлетворённость):**

- Уравнение: SWLS (удовлетворённость) = 22.16 + 0.86 × ProductivityIndex

- R² = 0.016 (1.6% дисперсии объясняется)

- Коэффициент ProductivityIndex: 0.86 (p=0.053)

- Предсказательная способность: слабая (R²=0.016)

- Интерпретация: чем выше ProductivityIndex (больше проактивности), тем выше SWLS (удовлетворённость)


![Регрессии ProductivityIndex](figures/productivity_index_regressions.png)


**Линейная регрессия с ProductivityIndex (типовая неделя, representative ∈ [-1, 1]):**

*Выборка: 154 из 231 респондентов (66.7%)*

| Показатель | Полная (β₁) | Полная (R²) | Полная (p) | Типовая (β₁) | Типовая (R²) | Типовая (p) | ΔR² |
| --- | --- | --- | --- | --- | --- | --- | --- |
| MBI (выгорание) | -2.997 | 0.111 | 0.0000 | -3.035 | 0.135 | 0.0000 | +0.024 |
| Прокрастинация | -2.041 | 0.077 | 0.0000 | -2.258 | 0.099 | 0.0001 | +0.021 |
| SWLS (удовлетворённость) | 0.858 | 0.016 | 0.0527 | 0.703 | 0.011 | 0.1994 | -0.005 |


![Регрессии ProductivityIndex — типовая неделя](figures/productivity_index_typical_week.png)


**Детальная статистика (типовая неделя):**

- **MBI (выгорание):** MBI (выгорание) = 23.13 + -3.03 × PI, R² = 0.135, p = 0.0000

  → Чем выше ProductivityIndex, тем ниже MBI (выгорание)

- **Прокрастинация:** Прокрастинация = 25.78 + -2.26 × PI, R² = 0.099, p = 0.0001

  → Чем выше ProductivityIndex, тем ниже Прокрастинация

- **SWLS (удовлетворённость):** SWLS (удовлетворённость) = 22.20 + 0.70 × PI, R² = 0.011, p = 0.1994

  → Чем выше ProductivityIndex, тем выше SWLS (удовлетворённость)


**Выводы по индексу ProductivityIndex:**

- Наиболее сильная связь: с MBI (r=-0.348, p=0.000)

- Чем выше ProductivityIndex (больше проактивности), тем ниже MBI

- Логарифмическая шкала по основанию 2 обеспечивает симметрию и интерпретируемость:

  значения +X и -X соответствуют равному по силе, но противоположному по направлению дисбалансу

  Значение +1 означает в 2 раза больше целевых, -1 — в 2 раза больше срочных

- При 7 кубиках и eps=1 диапазон ProductivityIndex составляет [-3, 3]

- ProductivityIndex может быть полезен как компактный индикатор продуктивного профиля

- Используемый eps = 1 обеспечивает диапазон [-3, 3] при логарифме по основанию 2


## Анализ времени ответа на странице 2 (распределение кубиков)


**Описание анализа:**

Время, затраченное респондентом на распределение кубиков по зонам, может быть
связано с когнитивной нагрузкой при принятии решения. Например:
- Быстрое решение может указывать на уверенность или интуитивный выбор
- Длительное размышление может свидетельствовать о внутренней борьбе или
  желании дать социально ожидаемый ответ

Здесь анализируется корреляция между временем на странице 2 (в секундах)
и количеством кубиков в каждой зоне.


**Выборка:** 230 респондентов из 231 завершённых

**Отброшено выбросов:** 1 респондентов с временем > 3300 сек (100 × медиана = 33 сек)


**Время на странице 2:**

- M = 41.7 сек, SD = 36.7

- Медиана = 33.0 сек

- Диапазон = 2 – 397 сек

- Q1 = 25 сек, Q3 = 47 сек


**Корреляции Спирмена: время vs кубики:**

- Срочное ●: r = -0.022, p = 0.7348 n.s. (слабая отрицательная)

- Целевое ●: r = 0.125, p = 0.0574 n.s. (слабая положительная)

- Операционное ●: r = -0.070, p = 0.2877 n.s. (слабая отрицательная)


![Время ответа vs кубики](figures/response_time_vs_cubes.png)


**Время ответа по уровням профиля:**

| Уровень | Профиль | n | M (сек) | SD |
| --- | --- | --- | --- | --- |
| 1 | Хаос (все зоны равны) | 20 | 36.0 | 16.4 |
| 2 | Выживание (К>З>С) | 21 | 44.7 | 21.5 |
| 3 | Кризис (К>С>З или С>К>З) | 71 | 37.3 | 28.7 |
| 4 | Не сдаёмся (З>К>С) | 20 | 34.5 | 15.4 |
| 5 | Рост (З>С>К) | 47 | 45.5 | 29.7 |
| 6 | Дзен (С>З>К) | 51 | 48.2 | 60.8 |


![Время ответа по уровням](figures/response_time_by_level.png)


**Тест Крускала-Уоллиса: различия времени по уровням:**

- H = 5.649, p = 0.3419

- **Результат не значим**: нет статистически значимых различий во времени ответа между уровнями профиля (p > 0.05)


**Выводы по анализу времени:**

- Наиболее сильная связь времени с зоной Целевое ●: r = 0.125, p = 0.0574

- Направление: больше кубиков — больше времени

- Значимых корреляций (p < 0.05) не обнаружено


## Проверка гипотез на типовой неделе


**Описание анализа:**

Один из источников шума в данных — нетипичные недели. Респонденты, у которых
неделя была «значительно хуже» или «значительно лучше» обычной, могут иметь
искажённое распределение кубиков, не отражающее их типичный паттерн.

Здесь проверяются те же гипотезы, но только для респондентов, которые указали,
что их неделя была близка к обычной (значение representative от -1 до 1 включительно).

Сравниваются коэффициенты корреляции Спирмена для полной выборки и для
подвыборки типовой недели.


**Полная выборка:** 231 респондентов

**Типовая неделя (representative ∈ [-1, 1]):** 154 респондентов (66.7%)


**Сравнительная таблица корреляций Спирмена:**


| Гипотеза | Полная (r) | Полная (p) | Типовая (r) | Типовая (p) | delta_r | Значимо? |
| --- | --- | --- | --- | --- | --- | --- |
| 🔴 → Прокрастинация | 0.230 | 0.0004 | 0.293 | 0.0002 | +0.063 | — |
| 🔴 → MBI | 0.233 | 0.0003 | 0.254 | 0.0015 | +0.021 | — |
| 🔴 → SWLS | -0.047 | 0.4746 | -0.050 | 0.5416 | +0.002 | — |
| 🟢 → SWLS | 0.210 | 0.0013 | 0.167 | 0.0380 | -0.043 | — |
| 🟢 → MBI | -0.377 | 0.0000 | -0.398 | 0.0000 | +0.021 | — |
| 🟢 → Прокрастинация | -0.285 | 0.0000 | -0.359 | 0.0000 | +0.074 | — |
| Записи → MBI | nan | nan | nan | nan | N/A | N/A |
| 🟢² → MBI | -0.377 | 0.0000 | -0.398 | 0.0000 | +0.021 | — |
| Баланс×🔴 → MBI | 0.213 | 0.0011 | 0.300 | 0.0002 | +0.086 | — |
| Дефицит → MBI | 0.362 | 0.0000 | 0.293 | 0.0002 | -0.068 | — |

*delta_r — изменение абсолютного значения корреляции при переходе к типовой неделе*


**Корреляции зон с валидационными шкалами (типовая неделя):**

| Зона | Шкала | r | p | Знач. |
| --- | --- | --- | --- | --- |
| Срочное ● | Прокрастинация | 0.293 | 0.0002 | *** |
| Срочное ● | SWLS | -0.050 | 0.5416 |  |
| Срочное ● | MBI | 0.254 | 0.0015 | ** |
| Целевое ● | Прокрастинация | -0.359 | 0.0000 | *** |
| Целевое ● | SWLS | 0.167 | 0.0380 | * |
| Целевое ● | MBI | -0.398 | 0.0000 | *** |
| Операционное ● | Прокрастинация | 0.125 | 0.1215 |  |
| Операционное ● | SWLS | -0.052 | 0.5220 |  |
| Операционное ● | MBI | 0.142 | 0.0779 |  |


![Сравнение корреляций](figures/typical_week_correlations.png)


**Выводы по анализу типовой недели:**

- При фильтрации по типовой неделе:

  - Усилились корреляции: 0

  - Ослабели корреляции: 0

  - Стали значимыми: 0

  - Потеряли значимость: 0

- Фильтрация по типовой неделе не привела к появлению новых значимых связей


## Заключение


**Примечание к интерпретации:**
- p < 0.05 — статистически значимый результат
- Размер эффекта: малый = 0.2, средний = 0.5, большой = 0.8
- Корреляции: слабая |r| < 0.3, средняя 0.3 ≤ |r| < 0.5, сильная |r| ≥ 0.5

---
*Отчёт сгенерирован автоматически*

